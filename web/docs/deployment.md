...
- `LOB_SECRET_NAME` and the `WEBPRESA_LOB_SENDER_*` vars were already correctly configured on Vercel Production (confirmed present via `vercel env ls production`) — no Vercel env var changes were needed for this cutover, only the Secrets Manager write above. The sender address vars have held the real business mailing address on both environments since Stage 22 (set directly, not copied from dev).
...
**Deployed and live in dev as of 2026-08-21** (prod not yet deployed). Seven new DynamoDB tables, one new secret, one new CDK stack (`WebpresaSesStack`), a second Vercel Cron schedule. Two real deploy-time issues were hit and fixed on the first dev rollout — both documented in full below since they'll recur on the prod deploy otherwise:
...
2. **The SNS subscription confirmation never reached the app** — Vercel Deployment Protection (enabled on this project) redirects every unauthenticated request, including SNS's own delivery POSTs, to Vercel's SSO gate at the edge before Next.js ever runs (confirmed directly: `curl -X POST .../api/webhooks/ses` 302s to `vercel.com/sso-api`). This is the exact same class of problem already documented below for the Stripe webhook. Fixed the same way: `WebpresaSesStack` now embeds the `vercel-protection-bypass` secret as a `?x-vercel-protection-bypass=` query parameter directly in the CDK-managed subscription URL (`cdk.SecretValue.secretsManager(...).unsafeUnwrap()` joined into the endpoint string) — no manual step, unlike Stripe/Lob's out-of-band dashboard registration, since this subscription is fully CDK-managed. A subscription created before this fix landed will sit at `PendingConfirmation` forever; re-run `aws sns subscribe --topic-arn <arn> --protocol https --notification-endpoint <url-with-bypass-param> --profile <env>` (or just redeploy the stack once the fix is in) to force a fresh confirmation attempt against the now-reachable endpoint. The orphaned old pending subscription left behind by the stack update doesn't need manual cleanup — SNS auto-expires unconfirmed subscriptions after a few days.
