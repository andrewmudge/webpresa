...
| `WebpresaDevVercelAccessStack` | ✅ us-east-1 | Two CDK-managed IAM policies (`webpresa-dev-vercel-data-access`, `webpresa-dev-vercel-compute-invoke`) attached to the existing `webpresa-vercel-dev` user; depends on `WebpresaDevDataStack`, `WebpresaDevScreenshotStack`, `WebpresaDevScanWorkflowStack`. Deployed 2026-07-24, replacing 5 hand-run inline policies — see "AWS credentials for Vercel" in `deployment.md` |
