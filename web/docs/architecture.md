...
| GSI: `industry-kind-index` | PK: `industryKind` (computed as `"{industry\|'general'}#{kind}"`, never part of the validated `StockImage` shape), SK: `createdAt` — newest-first; also the Phase 2 browse-by-industry/filter-by-kind query |
| GSI: `status-index` ⚠ | PK: `status` |
...
**Key structure**: `hero-sets/{industry}/{setId}/desktop.{ext}` and, when the admin also uploads a separate mobile image, `hero-sets/{industry}/{setId}/mobile.{ext}` (a `kind: 'hero'` `StockImage`, `web/lib/s3/stock-images.ts`'s `uploadStockHeroSet` — desktop and mobile are always independent uploads, never one cropped from the other; mobile is genuinely optional) and `general/{industry|'uncategorized'}/{imageId}.{ext}` (a `kind: 'general'` standalone image, unused by any auto-pick tier today — reserved for the Phase 2 browsable library). Every upload gets a fresh, never-reused key namespace — "replacing" a stock image is always a new upload + new `StockImage` record + archiving the old one, never an overwrite — so this feature never needs a `CreateInvalidation` call. Each object's `Cache-Control` is set to `public, max-age=31536000, immutable` at upload time.
...
- **Desktop and mobile hero images are always independent — never cropped from one another (corrected 2026-07-27, same day).** An earlier version of this stage auto-cropped one uploaded photo into both a desktop and a mobile variant; direct product feedback reversed this — uploading a desktop hero photo (or a stock hero image) must never generate or imply a mobile version, and vice versa. `Business.heroPhotoUrlMobileAuto` (the auto-crop pairing field) was removed from the model/schema entirely before it ever held real data. Mobile resolution's actual rule, applied uniformly across every tier: `heroPhotoUrlMobile === 'none'` forces no mobile photo; an explicit `heroPhotoUrlMobile` override always wins; otherwise a *genuinely separate, independently-uploaded* mobile image is used if one exists for that tier (only a stock hero set can have one, via its own optional mobile upload); otherwise mobile simply reuses whatever desktop photo resolved, as a preview — a real (if not mobile-cropped) photo beats no photo at all, but nothing is ever cropped to produce it. `lib/image/crop-hero-photo.ts` was deleted; no cropping happens anywhere in this feature. See `build_log.md`, "Correction: desktop and mobile hero images are independent," for the full record.
