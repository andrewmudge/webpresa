...
Two shortcuts skip the AI call entirely: no website on file (`ScanEvent` marked `'completed'`, `Business.qualification` forced to `'qualified'`) and a Firecrawl scan that failed as unreachable/blocked/invalid (`'manual_approval_required'`, forced to `'manual_review'`).
