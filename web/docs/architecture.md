...
| `WebpresaDevVercelAccessStack` | ✅ us-east-1 (redeployed 2026-07-27 for the new S3StockImages grant) | Two CDK-managed IAM policies (`webpresa-dev-vercel-data-access`, `webpresa-dev-vercel-compute-invoke`) attached to the existing `webpresa-vercel-dev` user; depends on `WebpresaDevDataStack`, `WebpresaDevScreenshotStack`, `WebpresaDevScanWorkflowStack`, `WebpresaDevStockImagesStack`. Deployed 2026-07-24, replacing 5 hand-run inline policies — see "AWS credentials for Vercel" in `deployment.md` |
| `WebpresaProdScreenshotRepositoryStack` | ❌ not deployed | Same, prod settings |
| `WebpresaProdScreenshotStack` | ❌ not deployed | Same, prod settings |
| `WebpresaProdScanWorkflowStack` | ❌ not deployed | Same, prod settings |
| `WebpresaProdStockImagesStack` | ❌ not deployed | Same, prod settings |
| `WebpresaProdVercelAccessStack` | ❌ not deployed | Same, prod settings — attaches to `webpresa-vercel-prod` once that user exists |
...
| `INTERNAL_API_SECRET_NAME` | Deterministic — `webpresa-dev-internal-api` | Secrets client (`web/lib/secrets/`) — Stage 16 internal-API auth |
