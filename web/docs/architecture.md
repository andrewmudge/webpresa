...
- **Hero tier chain rewrite (Phase 1 stock image repository, 2026-07-27)** — the hero slot's automatic resolution moved out of `resolvePhotoSlot`/inline dimension-checking in `lib/ai/generate-preview.ts` and into a dedicated `resolveHeroImages()` (`lib/image/resolve-hero-image.ts`), with a new chain: (1) explicit admin override — unchanged, and now the desktop hero's direct-upload input auto-crops to the exact full-bleed dimensions with a paired mobile crop (`Business.heroPhotoUrlMobileAuto`, internal, never admin-set); (2) a Firecrawl-discovered `role: 'hero'` image whose *already-stored* dimensions are within tolerance of 1920×1080 (16:9) — new, reuses `HERO_FULL_BLEED_DIMENSIONS`/`HERO_DIMENSION_TOLERANCE_PX`; (3) a curated stock hero image matched to `Business.industry` — new, see "Stock image CDN" above; (4) theme illustration. The old "first uploaded business photo" auto-fallback tier is **removed** — uploaded photos are hero-selectable only via explicit override now. Mobile hero also gained its first automatic fallback, pairing with whichever tier supplied the desktop image (a Firecrawl-tier hero has no mobile companion, so mobile still falls through to the illustration in that case unless manually overridden). The about/services/aboutSection slots' own `resolvePhotoSlot` chains are untouched. See `build_log.md` for the full record.
