...
- **Template polish and manual social links (2026-07-19):** five items from direct UI feedback.
  - **Hero split-image panel** (`GeneratedHero.tsx`'s `HeroCornerImage`, shared by the `'illustration'`/`'imageSplit'` styles and `'image'`'s mobile rendering) now floats as a fully rounded (`rounded-2xl`), shadowed (`shadow-xl`) card inset from the section's top/right/bottom edges (`inset-y-4 right-4` mobile, `lg:my-8 lg:mr-8 xl:mr-12` desktop) instead of a flat, unshaded edge-to-edge fill. Only the left edge stays flush against the text column, blended via the existing gradient overlay rather than gapped, so it still reads as one continuous section.
