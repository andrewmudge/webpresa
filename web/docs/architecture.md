...
**Not yet implemented.** This section documents the finalized architectural boundary Stage 12 must respect once built, so this document, `implementation.md`, and `deployment.md` stay consistent. See `implementation.md`, Stage 12, for the full requirements, field mapping, and acceptance criteria.

- **Server-only calls.** Google Places requests follow the same pattern as every other third-party integration in this codebase: a `server-only` client reads its API key from `getGooglePlacesSecret()` (`web/lib/secrets/index.ts`), and is only ever invoked from a Server Action or Route Handler that itself verifies the admin session — never from a client component, never from a public endpoint.
- **Secret already provisioned.** The `webpresa-{env}-google-places` secret (`{ apiKey }`) was created in Stage 10 (`infra/lib/stacks/data-stack.ts`, `WebpresaSecret` construct) and is already granted to `webpresa-vercel-dev`'s IAM policy and readable via `GOOGLE_PLACES_SECRET_NAME`. Stage 12 is its first real caller — no new secret, IAM grant, or environment variable is needed for the key itself.
- **Existing model and repository, no new table.** Imported records become ordinary `Business` records via the existing `web/lib/db/businesses.ts` repository (`putBusiness`, `getBusinessById`, `getBusinessBySlug`, `resolveUniqueSlug`). No separate `Prospect` aggregate or DynamoDB table is introduced. The `webpresa-dev-businesses` table's existing `google-place-id-index` GSI (`infra/lib/stacks/data-stack.ts`) is the fast-path duplicate check for Google Place ID; other duplicate signals (domain, phone, name+address) are checked in application code — see `implementation.md`, Stage 12, "Server-side duplicate detection."
- **Transient search results.** Google Places search-result objects exist only in server memory and the review UI's state until an admin explicitly selects and imports them. Nothing is written to DynamoDB or S3 during search or review — only at the moment of a selective import.
- **No Google photo retrieval.** Google Places photo binaries are never retrieved or stored by Stage 12. `businesses/{businessId}/assets/` (Stage 9) remains reserved for admin-uploaded or otherwise explicitly approved business assets only — Stage 12 never writes to it. `scans/{businessId}/{scanId}/` remains reserved for Stage 13's Firecrawl artifacts and Stage 14's Playwright artifacts; Stage 12 writes no scan artifacts and does not write to that prefix.
...
- **Source-attributed provider facts.** `Business.googleRating` / `Business.googleReviewCount`, once populated by Stage 12, are Google-sourced facts, not manually entered reviews or generic testimonials — see "Section eligibility signals" above.
...
- **No status queue.** An imported business's `status` is the existing `'pending'` value from `BUSINESS_STATUSES` (`domain/models/business.ts`) — the same default `createBusiness()` already assigns. There is no `READY_FOR_SCAN` or `READY_FOR_ENRICHMENT` status anywhere in the model. Starting Stage 13 enrichment is a distinct, explicit admin action on the business detail page, not a queue or status Stage 12 populates.
