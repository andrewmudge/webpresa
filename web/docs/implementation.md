...
### Documentation gap — resolved, confirmed end to end (2026-08-29)

Everything in this section was originally a best-effort guess against gated/incomplete docs; all of it is now confirmed against real behavior, culminating in a full real purchase (`ctest.info`, PTE) that succeeded end to end on the first attempt with the code described below:

- **`createStorefrontCustomer()`** — `POST /v1/customer` (singular), requiring `address1`/`city`/`state`/`postal_code`/`country`/`phone` in addition to name/email/username; returns a bare UUID string, not `{customer_id: ...}`. `username` is capped at 20 chars — too short for `webpresa_<36-char-sub>` (the original design), fixed via `deriveStorefrontUsername()`'s short deterministic hash.
- **Authentication is OAuth 2.0 client-credentials, not a raw API key as a Bearer token** — an early version of this integration got `401 invalid authentication token` sending the raw key directly. The real flow: exchange `clientId`+`apiKey` (the latter functioning as the OAuth Client Secret) via HTTP Basic Auth for a short-lived access token at `POST https://auth.test.shopco.com/oauth2/token` (PTE) / `https://auth.shopco.com/oauth2/token` (live) with `grant_type=client_credentials`, then use *that* token as the Bearer credential (`lib/opensrs/client.ts`'s `getAccessToken()`, cached per-process with a 30s expiry margin). `OPENSRS_STOREFRONT_API_BASE_URL` is `https://api.test.shopco.com` (PTE) / `https://api.shopco.com` (live).
- **The webhook's real payload shape**: `{ event, event_id, username, changes_by, is_success, domain_name, created_date, record_type, data }`, with `event: "Domain registration"` verbatim for a genuine registration. **Critically, there is no `external_user_id`/`extuserid` field anywhere in the payload** — the original design's correlation mechanism (round-tripping a generated id through Storefront's `extuserid` query param) doesn't work at all, since it's simply never sent back. Reworked around `username` instead: `DomainPurchaseIntent` stores `storefrontUsername` (deterministic per Cognito `sub`) and gains a `storefront-username-index` GSI so the webhook can look up the most recent `'pending'` intent for whichever username the payload carries.
- **Signature verification**: header `x-signature`, format `sha256=<hex digest>` (the `sha256=` prefix must be stripped before comparing) — not the originally-guessed `x-opensrs-signature` header with a bare digest.
- Storefront's own docs confirm a webhook subscribes per *category* (not per individual event) and receives every event in it — `isLikelyRegistrationEvent()` in the route handler filters via `record_type === 'domain'` + a case-insensitive substring match on `"registration"` in `event`, deliberately not an exact-string compare, since other Domain Events (DNS changes, nameserver updates, etc.) will also arrive at this same endpoint.

Also worth flagging, still a real known gap: `resolveOpenSrsCustomerId()` (`app/app/onboarding/[businessId]/actions.ts`) sources the required address/phone from the *Business's* own contact info (this app has no personal-contact-info source for the customer as a person) and passes `phone` through unvalidated — OpenSRS requires E.164 format and this app doesn't normalize phone numbers anywhere today. A Business missing address/phone is blocked from starting a purchase with a clear redirect; a malformed (non-E.164) phone number will only surface as a real API error at request time.
...
- OpenSRS Storefront credentials are never sent to the browser, logged, or returned outside authorized pages. No *registrant* (domain WHOIS) contact data is ever collected or stored by Webpresa — Storefront collects and holds that entirely on its own side at checkout. The Business's own address/phone (already-public business contact info, not personal/sensitive data) is sent to OpenSRS at customer-account-creation time only, since Storefront's customer API requires it.
...
- ~~**Lob live mode**~~ — done 2026-08-19: live API key + live webhook (`https://webpresa.com/api/webhooks/lob`, no protection-bypass query param needed — Production isn't behind Vercel Deployment Protection) written into `webpresa-prod-lob`. See `deployment.md`, "Lob live mode setup," for the full record, including a discovered event-type gap (`postcard.mailed` wasn't selectable in the Live webhook picker, only `postcard.delivered`) accepted as-is. `SubmitButton.tsx` updated to show live-vs-test-accurate copy (`isLobLiveMode()`, `web/lib/lob/client.ts`). Pending: the user's own Production redeploy to activate these values, and a live end-to-end smoke test (deliberately deferred).
...
Complete and deployed to both dev and prod (2026-08-21). `npm run lint`, `npx tsc --noEmit`, `npm test` (157 test files, 1644 tests web-side; 12 test files, 240 tests infra-side), and `npm run build` all pass. New infrastructure (deployed to both accounts): 7 DynamoDB tables, 2 Secrets Manager secrets-worth of grants (a new `marketing-click-token` secret — independently-generated value per environment — plus a second IAM managed policy — `MarketingDataAccessPolicy` — split out after the original `DataAccessPolicy` exceeded IAM's 6,144-byte size limit on first deploy), and a new CDK stack (`WebpresaSesStack` — Configuration Set, SNS topic, HTTPS subscription, with the Vercel-protection-bypass secret embedded in the subscription URL after Vercel's Deployment Protection was found to block SNS's confirmation POST at the edge on dev/Preview — confirmed inert but harmless on Production, which isn't behind that gate). Also added: a miniature outbox (`/admin/marketing/outbox`, linked from the "Email N Sent" KPI cards) showing the actual rendered content of every sent email, with `MarketingMessage.recipientEmail` now snapshotted at send time for the same audit reasons subject/body already were. `main` was fast-forwarded to match `dev` (no divergence, no force-push needed) to ship all of this to production. See `deployment.md`, "Marketing stage — SES Drip Campaign deployment guidance" for the full record of both fixes and the prod deploy sequence.
