...
`createStorefrontCustomer()`/`getStorefrontSsoUrl()` and their real endpoint paths, request/response shapes, and required fields (`POST /v1/customer` — singular, not plural; requires `address1`/`city`/`state`/`postal_code`/`country`/`phone` in addition to name/email/username; returns a bare UUID string, not `{customer_id: ...}`) are all confirmed (2026-08-28) against OpenSRS's real public support docs ("API: Managing Customers"), and `OPENSRS_STOREFRONT_API_BASE_URL` is confirmed as `https://api.test.shopco.com` (PTE) / `https://api.shopco.com` (live). `Authorization: Bearer <apiKey>` auth is also confirmed correct against a real documented endpoint example.

**Still unconfirmed**: the webhook's exact payload schema and signature-verification mechanics — the real article ("Storefront Webhook Notifications") stayed gated behind reseller-support login throughout this work. What's confirmed instead (from OpenSRS's public support-site search index): webhooks are configured under Storefront Settings → Advanced Settings → Add Webhook, with **Order Events** (including "Completed order") and **Domain Events** (including "Domain registration") as real event categories, and registering one returns a "webhook key." The PTE webhook is registered live (Domain Events → "Domain registration") and the real webhook key is populated in `webpresa-dev-opensrs-storefront`. `lib/opensrs/verify-webhook.ts` and the route handler's `extractEventType`/`extractDomain`/`extractOrderId`/`extractIntentId` functions still implement a reasonable best-effort shape (HMAC-SHA256 over the raw body; `event_type`/`domain`/`order_id`/`external_user_id` JSON fields) that **must be confirmed against a real delivered webhook** before this part is considered done — trigger a real PTE test purchase, inspect the actual delivered headers/body, and adjust.

Also worth flagging: `resolveOpenSrsCustomerId()` (`app/app/onboarding/[businessId]/actions.ts`) sources the required address/phone from the *Business's* own contact info (this app has no personal-contact-info source for the customer as a person) and passes `phone` through unvalidated — OpenSRS requires E.164 format and this app doesn't normalize phone numbers anywhere today. A Business missing address/phone is blocked from starting a purchase with a clear redirect; a malformed (non-E.164) phone number will only surface as a real API error at request time.
