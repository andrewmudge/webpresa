...
try {
  ...
  const result = await page.evaluate(() => {
    ...
    const phoneSvg = Array.from(frame.querySelectorAll('svg')).find((s) => s.getAttribute('viewBox') === '0 0 406 866');
    const phoneRect = phoneSvg.getBoundingClientRect();
    const safeXPercent = ((0.125 + 0.125) / 9.25) * 100;
    const safeYPercent = ((0.125 + 0.125) / 6.25) * 100;
    const safeRightX = frameRect.right - frameRect.width * (safeXPercent / 100);
    const safeTopY = frameRect.top + frameRect.height * (safeYPercent / 100);
    const laptopCenterX = (laptopRect.left + laptopRect.right) / 2;
    const midpointX = (laptopRect.right + safeRightX) / 2;
    const phoneCenterX = (phoneRect.left + phoneRect.right) / 2;
    return {
      frame: { left: frameRect.left, top: frameRect.top, width: frameRect.width, height: frameRect.height },
      laptop: { left: laptopRect.left, right: laptopRect.right, top: laptopRect.top, width: laptopRect.width },
      featureRow: { left: featureRowRect.left, right: featureRowRect.right, top: featureRowRect.top, bottom: featureRowRect.bottom, height: featureRowRect.height, centerX: (featureRowRect.left + featureRowRect.right) / 2 },
      laptopCenterX,
      safeTopY,
      safeRightX,
      midpointX,
      ...
      gapAboveFeatureRow: featureRowRect.top - safeTopY,
      gapBelowFeatureRow: laptopRect.top - featureRowRect.bottom,
      featureRowCenterOffsetFromLaptopCenter: ((featureRowRect.left + featureRowRect.right) / 2) - laptopCenterX,
      phoneCenterOffsetFromMidpoint: phoneCenterX - midpointX,
      phoneRightVsSafeLine: safeRightX - phoneRect.right,
    };
  });
