...
describe('wasActiveAsOf', () => {
  it('false when firstPaidAt is unset', () => {
    expect(wasActiveAsOf(makeBusiness({ firstPaidAt: undefined, subscriptionStatus: 'active' }), '2026-08-01T00:00:00.000Z')).toBe(false);
  });

  it('false when firstPaidAt is after the as-of instant', () => {
    const b = makeBusiness({ firstPaidAt: '2026-09-01T00:00:00.000Z', subscriptionStatus: 'active' });
    expect(wasActiveAsOf(b, '2026-08-01T00:00:00.000Z')).toBe(false);
  });

  it('true when currently active, regardless of a stale canceledAt from a prior cycle', () => {
    const b = makeBusiness({ firstPaidAt: '2026-01-01T00:00:00.000Z', subscriptionStatus: 'active', canceledAt: '2026-03-01T00:00:00.000Z' });
    expect(wasActiveAsOf(b, '2026-08-01T00:00:00.000Z')).toBe(true);
    ...
    expect(wasActiveAsOf(b, '2026-02-01T00:00:00.000Z')).toBe(true);
  });
  ...
  it('true for a canceled business at a point in time before its cancellation', () => {
    ...
    expect(wasActiveAsOf(b, '2026-03-01T00:00:00.000Z')).toBe(true);
  });

  it('false for a canceled business at a point in time after its cancellation', () => {
    const b = makeBusiness({ firstPaidAt: '2026-01-01T00:00:00.000Z', subscriptionStatus: 'canceled', canceledAt: '2026-06-01T00:00:00.000Z' });
    expect(wasActiveAsOf(b, '2026-07-01T00:00:00.000Z')).toBe(false);
  });

  it('false for past_due (never counted as active for MRR/health purposes)', () => {
...
describe('countActiveAsOf / mrrCentsAsOf', () => {
  ...
  it('counts only businesses active as of the given instant', () => {
    expect(countActiveAsOf(businesses, '2026-08-01T00:00:00.000Z')).toBe(2);
  });

  it('sums normalized monthly cents across active businesses', () => {
    expect(mrrCentsAsOf(businesses, '2026-08-01T00:00:00.000Z')).toBe(3900 + 3125);
  });
});
